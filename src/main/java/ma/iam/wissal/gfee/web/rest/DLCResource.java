...
/**
 * REST controller for managing {@link ma.iam.wissal.gfee.domain.DLC}.
 */
@RestController
@RequestMapping("/api")
public class DLCResource {
	...
	@GetMapping("/dlcs/by-direction/{idDirection}")
	public ResponseEntity<List<DLCDTO>> getDlcByDirection(@PathVariable Long idDirection) {
		log.debug("REST request to get DLC by direction : {}", idDirection);
		try {
			List<DLCDTO> dlcDtos = dLCService.findDlcsByDirection(idDirection);
			if (dlcDtos == null || dlcDtos.isEmpty()) {
				return ResponseEntity.notFound().build();
			}

			return ResponseEntity.ok(dlcDtos);
		} catch (Exception e) {
			log.error("Error when getting DLC by direction", e);
